...
namespace LittleSheep.ControlModule
{
    ...
    class ControlMSG
    {
        ...
        public static byte[] GetKeyMsgByte(KeyboardCode bVk, KeyStatus status) {
            byte[] msg = new byte[2];
            msg[0] = (byte)bVk;
            msg[1] = (byte)status;
            return msg;
        ...
        public static byte[] GetMouseMsgByte(MouseCode bvk, MouseState status) {
    ...
    class KeybdHook
    {
        ...
        private static extern int SetWindowsHookEx(int idHook, LowLevelKeyboardProcHandler lpfn, IntPtr hmod, int dwThreadId);
        ...
        /// 否则，其他安装了WH_MOUSE_LL挂钩的应用程序将不会收到挂钩通知，因此可能会出现错误行为。
        /// 如果钩子过程处理了消息，它可能会返回一个非零值，以防止系统将消息传递给钩子链的其余部分或目标窗口过程。</returns>
        private int KaybdHookProc(int nCode, UIntPtr wParam, IntPtr lParam) {
            ...
            if(nCode < 0) {
                ...
                switch(keybdEventIdenti) {
                    case KEYBDEVENT.WM_KEYDOWN:
                        // 键盘键按下
                        KeybdMsgEvent($"{MyKeybdHookStruct.vkCode}被按下");
                        break;
                    ...
                    case KEYBDEVENT.WM_KEYUP:
                        // 键盘键松开
                        KeybdMsgEvent($"{MyKeybdHookStruct.vkCode}被松开");
                        break;
